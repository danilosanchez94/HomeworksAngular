...
export class ProductListComponent implements OnInit {
  ...
  private readonly filterOfferSubject$$: Subject<boolean> =
    new Subject<boolean>();
  private readonly filterOffer$: Observable<boolean> =
    this.filterOfferSubject$$.asObservable();

  private readonly AddNewSubject$$: Subject<Product | null> =
    new Subject<Product | null>();
  private readonly AddNew$: Observable<Product | null> =
    this.AddNewSubject$$.asObservable();
  ...
  openDialog(): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      const keyValueObject: { [tag: string]: number } = result.prices.reduce(
        (acc: { [x: string]: number }, obj: { tag: string; price: number }) => {
